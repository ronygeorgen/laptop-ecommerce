...
class Dashboard(View):
    def monthly_earnings(self):
        ...
        monthly_earnings = (
            Order.objects.filter(created_at__range=(start_date, end_date)).aggregate(
                total_earnings=Sum("order_total")
            )["total_earnings"]
            or 0
        )
    ...
    def get(self, request):
        ...
        revenue = (
            orders.aggregate(total_revenue=Sum("order_total"))["total_revenue"] or 0
        )
        ...
        categories_count = (
            Variations.objects.values("product__category").distinct().count()
        )
        ...
        sorted_product_attributes = sorted(
            product_attribute_counter.items(), key=lambda x: x[1], reverse=True
        )
        ...
        order_products = (
            ...
            .values(
                "variations__id",
                "variations__product__product_name",
                "variations__brand_name",
                "variations__color",
                "variations__ram",
                "variations__storage",
                "variations__price",
        ...
        sorted_product_attributes_for_count = sorted(
            order_products, key=lambda x: x["count"], reverse=True
        )
        context = {
            "revenue": revenue,
            "count": count,
            ...
            "users": Account.objects.filter(is_admin=False).order_by("-date_joined")[
                :5
            ],
            "month": chart_month,
            "new_users": new_users,
...
class OrderDetailView(View):
    def get(self, request, pk):
        ...
        return render(
            request, "admin_templates/evara-backend/page-orders-detail.html", context
        )
...
class SalesReportView(View):
    def get(self, request):
        ...
        return render(
            request, "admin_templates/evara-backend/sales_report.html", context
        )
