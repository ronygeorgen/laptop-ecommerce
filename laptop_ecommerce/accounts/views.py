...
class UserDashboardView(View):
    def get(self, request):
        ...
        orders = OrderProduct.objects.order_by("-created_at").filter(
            user_id=request.user.id, ordered=True
        )
    ...
    def get(self, request):
        ...
        for order in orders:
            order.total_quantity = (
                OrderProduct.objects.filter(order=order.order, ordered=True).aggregate(
                    Sum("quantity")
                )["quantity__sum"]
                or 0
            )
...
class MyOrdersDetailedView(View):
    def get(self, request, order_id, pk):
        orders = OrderProduct.objects.filter(
            order__order_number=order_id, user=request.user, ordered=True
        ).order_by("-created_at")
        for_address = OrderProduct.objects.filter(
            id=pk, user=request.user, ordered=True
        )
    ...
    def post(self, request):
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                user.save()
                messages.success(
                    request, "Password updated successsfully. Please login again"
                )
                logout(request)
                return redirect("login")
